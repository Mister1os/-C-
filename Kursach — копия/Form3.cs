...
namespace Kursach
{
    public partial class Form3 : Form
    {
        ...
        private void ApplyTheme()
        {
            ...
            {
                // Тёмная тема
                this.BackColor = Color.FromArgb(115, 38, 127);
        ...
        private void FillFormFromSelectedRow()
        {
            ...
            switch (currentTable)
            {
                ...
                case "Аккаунты":
                    ...
                    {
                        MessageBox.Show("У вас нет прав доступа для этой таблицы");
                    }
                        break;
        ...
        {
            // Скрываем все элементы управления аккаунтами для сотрудников
            if (currentTable == "Аккаунты" && !HasPermission("Администратор"))
            {
                MessageBox.Show("У вас нет прав для управления аккаунтами");
                currentTable = "";
                return;
            }

            // Скрываем кнопки управления для сотрудников
            pictureBox2.Visible = HasPermission("Администратор") || currentTable != "Аккаунты";
            pictureBox3.Visible = HasPermission("Администратор") || currentTable != "Аккаунты";
            pictureBox4.Visible = HasPermission("Администратор") || currentTable != "Аккаунты";
            ...
            switch (currentTable)
            {
                ...
                case "Категории":
                    textBox2.Visible = true; // Название категории
                    break;
        ...
        private void EditRecord()
        {
            ...
            try
            {
                ...
                switch (table)
                {
                    ...
                    case "Аккаунты":
                        if (!HasPermission("Администратор"))
                        {
                            MessageBox.Show("Только администратор может редактировать аккаунты");
                            return;
                        }
                        ...
                        command.Parameters.AddWithValue("@role", textBox4.Text.Trim());
                        command.Parameters.AddWithValue("@id", id);
                        break;
            ...
            try
            {
                ...
                switch (table)
                {
                    ...
                    case "Аккаунты":
                        if (!HasPermission("Администратор"))
                        {
                            MessageBox.Show("Только администратор может добавлять аккаунты");
                            return;
                        }
                        if (string.IsNullOrWhiteSpace(textBox2.Text) || string.IsNullOrWhiteSpace(textBox3.Text) || string.IsNullOrWhiteSpace(textBox4.Text))
                        {
        ...
        private void HighlightSelectedLabel(System.Windows.Forms.Label selectedLabel)
        {
            if (darkMode == false) {
                ...
                selectedLabel.ForeColor = Color.FromArgb(17, 155, 26);
